...
#   }
#
# The `invoice_number` is generated by the backend on create and persisted on
# both the write model and the read model. Format: INV-YYYYMMDD-NNNN, where
# NNNN is a per-day sequence. It is unique and safe to display to end users.
...
Create many sales in one request by uploading a CSV file. Each unique
`import_ref` in the file becomes one Sale aggregate (with 1..N line items);
the backend generates one `invoice_number` per created sale. The `import_ref`
column is **only a grouping key** — it is never persisted.

**Endpoint:** `POST /api/sales/import-csv`
**Content-Type:** `multipart/form-data`
**Form field:** `file` (CSV, max 2 MB, mime `text/csv` or `text/plain`)

### CSV columns (header required)

| Column        | Required | Notes                                              |
|---------------|:--------:|----------------------------------------------------|
| `import_ref`  |    ✓     | Grouping key (e.g. `sale-001`). Not persisted.     |
| `customer_id` |    ✓     | ULID. Must exist and be consistent per group.      |
| `product_id`  |    ✓     | ULID. Must exist in the product catalog.           |
| `quantity`    |    ✓     | Positive integer.                                  |

A ready-to-use sample file lives at
[`Tests/Http/fixtures/sales-import-sample.csv`](./fixtures/sales-import-sample.csv)
(see [`Tests/Http/fixtures/README.md`](./fixtures/README.md)).
...
```json
{
  ...
  "failed_rows": 0,
  "sales": [
    { "import_ref": "sale-001", "sale_id": "01ARZ...", "invoice_number": "INV-20260910-0001" },
    { "import_ref": "sale-002", "sale_id": "01ARZ...", "invoice_number": "INV-20260910-0002" },
    { "import_ref": "sale-003", "sale_id": "01ARZ...", "invoice_number": "INV-20260910-0003" }
  ],
  "errors": []
}
```

### Failure (422 — fail-entire-import)

If any row fails validation, no sales are persisted and the response lists
every offending row:
...
```json
{
  ...
  "sales": [],
  "errors": [
    { "row": 3, "import_ref": "sale-001", "field": "quantity",    "message": "quantity must be a positive integer" },
    { "row": 6, "import_ref": "sale-003", "field": "customer_id", "message": "customer not found" }
  ]
}
```

Row numbers are 1-based including the header (so row `2` is the first data
row). The domain aggregate rules still apply per group — for example all
rows sharing the same `import_ref` must reference the same `customer_id`.
